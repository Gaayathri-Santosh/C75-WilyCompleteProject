...
export default class Searchscreen extends React.Component {
  ...
  searchTransactions = async () => {
    var searchText = this.state.searchText.toLowerCase() //Changing the textinput case
    var first_Alphabet = searchText.split("") //To identify whether it is bookid or studentid 
    console.log("42:", first_Alphabet)
    if (first_Alphabet[0] === 'b') {
      const transaction = await db.collection("transactions").where('bookid', '==', this.state.searchText).limit(5).get()
      transaction.docs.map((doc) => {//This ll always be an array .startAfter(this.state.lastVisibeTransaction)
        this.setState({
          allTransactions: [...this.state.allTransactions, doc.data()],//It is appending the doc data
          lastVisibleTransaction: doc
        })
